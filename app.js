...
app.get('/movies_orderByName/:num', function (req, res) {
    ...
    + `SELECT distinct posterImg, r_movieCode, r_movieTitle FROM Movies m, Recommendation r WHERE m.movie_code = r.movie_code and m.movie_code = ${mcode};`
    ...
    conn.query(sql, function (err, rows, fields) {
        ...
        if(err) console.log('query is not excuted. select fail...\n' + err);
            ...
            rows[6].map((row) => {
                console.log(row);
                recommendList.push(row)
            })
            
            res.render('movieInfo.ejs', { basicInfo, scopeList, countryList, directorList, castList, castInfoList, photoInfoList, videoInfoList, rateList, filmoList, recommendList })
...
// 관련영화추천의 영화 클릭했을 때 이벤트 처리
app.get('/movieInfo/:num', function (req, res) {
...
router.route('/movieInfo_title').post(
    ...
    conn.query(sql, function (err, rows, fields) {
        ...
        if(err) console.log('query is not excuted. select fail...\n' + err);
            ...
            res.render('movieInfo_title.ejs', { basicInfo, scopeList, countryList, directorList, castList, castInfoList, photoInfoList, videoInfoList, rateList, filmoList, recommendList })
